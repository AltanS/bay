"""Tests for GH-13: image-map.json render task tags and webhook handler.
...
These are the regression guards required by the producer/pull-signal fanout spec.
"""
...
def test_image_map_render_runs_under_deploy_stack_M90_GH_13():
    """GH-13: the image-map.json render must execute under
    ...
    assert not missing, (
        ...
        f"tag in particular is the deploy_stack-tag fix — dropping it re-introduces "
        f"the stale image-map bug from the storefront cutover."
    )
...
def test_image_map_render_task_file_exists_M90_GH_13():
    ...
    assert render_file.is_file(), (
        f"GH-13: {render_file} is missing. The fix extracts the "
...
def test_webhook_restart_handler_exists_M90_GH_13():
    """GH-13: git_deploy must define a handler that restarts the
...
def test_image_map_template_notifies_webhook_restart_M90_GH_13():
    """GH-13: the image-map.json template task must notify the
    ...
    assert template_task is not None, (
        ...
        "image-map.json.j2. The render task is broken."
    )
