"""Cross-region link port-map resolution tests (M87 / GH bay#7).

`deploy.yml` "Build link port map" resolves `LINKS_*_PORT` for cross-region
service-to-service links. Pre-M87 it sourced the target spec from
`active_services|combine(active_accessories)` — the local-region active set —
so cross-region targets always fell through to 0. M87 changed the lookup to
...
def _resolve_port(
    ...
    versus "what the global services dict carries" — the M87 fix is that
...
class TestCrossRegionPortResolution:
    def test_cross_region_service_target_resolves_via_unfiltered_services(self):
        ...
        not contain `storefront-platform` (which is EU-only). Pre-M87
        this returned 0; post-M87 it resolves to 5100 from the global
        services dict."""
        services = {
            "storefront-platform": {
                "regions": ["eu"],
    ...
    def test_same_region_link_still_resolves(self):
        ...
        both global and active dicts must continue to resolve. The M87
        change must not break the existing path."""
        services = {
            "consumer": {
                "regions": ["eu"],
...
class TestPreM87RegressionGuard:
    def test_active_filter_path_no_longer_used_for_cross_region(self):
        ...
        active_services again, this fails. The whole point of M87 is
        ...
        # Strip comment lines so a "pre-M87 was X" historical note in
        ...
        assert "active_services" not in task_block, (
            "M87 regression: `Build link port map` must not reference "
