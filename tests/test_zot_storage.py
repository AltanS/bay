...
def test_build_specs_zot_spec_gates_env_file_on_s3() -> None:
    """GH bay#14: `_zot_spec` shipped without env_file in M71, so S3
    deployments started with no AWS_* and restart-looped with
    `s3aws: NoCredentialProviders`. The M71 spec must gate env_file on
