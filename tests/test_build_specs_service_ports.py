"""Service builder host-port rendering tests (M88 / GH bay#7).

`roles/container_lifecycle/tasks/build_specs.yml` is the path that produces
the spec dict consumed by `community.docker.docker_container`. M87 fixed
the compose-file render path (`_macros.j2`) but missed this one — the
container module got `ports: omit` for services regardless of whether
`ports.expose:` was declared. M88 adds the same `expose:` resolution to
...
Critical regression guards (per counsel):
  ...
  - The `.get()` chain protects against `_svc.ports` being undefined.
    The same trap (M86 incident) on `dict.attr | default(...)` falling
...
def _render_service_spec(svc: dict, headscale_server_tailnet_ip: str = "100.64.0.1") -> dict:
    """Re-implement the M88 service-spec render expression from
    build_specs.yml.
...
class TestServicePortsRendering:
    ...
    def test_storefront_platform_user_bug_pattern(self):
        ...
        spec = _render_service_spec(svc, headscale_server_tailnet_ip="100.64.0.1")
        assert spec.get("ports") == ["100.64.0.1:5100:5100"], (
            "M87 regression: the user's storefront-platform on EU did not "
...
class TestServiceWithoutPortsExpose:
    ...
    def test_no_ports_block_at_all_does_not_crash(self):
        ...
        `ports`. The `.get()` chain protects against this — the M86
...
class TestBuildSpecsYmlSource:
    ...
    accidentally drops the M88 lines fails immediately."""
    ...
    def test_service_block_uses_get_chain_not_attr_default(self):
        ...
        explicitly documents this trap (M86 incident, env.clear case)."""
        src = self._load()
