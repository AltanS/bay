"""Tests for M75-S3: rebuild.sh.j2 template rendering with remote strategy.
...
class TestPullOnlyServiceRendering:
    """M75-S6: pull-only services in rebuild.sh.j2."""
...
# ── Health check and rollback (M78-S1) ─────────────────────────────────


class TestHealthCheckAndRollback:
    """M78-S1: post-restart health check and automatic rollback in rebuild.sh."""
...
# ── Build duration monitoring and timeout (M78-S2) ─────────────────────
...
class TestBuildTimeout:
    """M78-S2: systemd timeout and OnFailure alert unit."""
...
class TestBuildDuration:
    """M78-S2: build duration tracking in rebuild.sh success notifications."""
...
# ── M75-S8: _wait_healthy StartPeriod awareness + timeout inversion fix ────


class TestWaitHealthyStartPeriodFix:
    """M75-S8: consecutive-unhealthy guard, default timeout bump, per-service override."""
