"""Tests for cross-region webhook fan-out and pull signal handling.
...
- Pull signal — app.py writes "pull" to trigger, no fan-out,
...
class TestPullSignal:
    """Pull signals from build server to deployment servers."""
...
# ── Image-level pull signal handling ─────────────────────────────


class TestImageLevelPullSignal:
    """Image-level pull signals at /webhook/pull-image."""
    ...
    def test_producer_with_siblings_all_receive_pull_M90_GH_13(self, webhook_env):
        """GH-13 regression guard: an inbound image-level pull webhook
        for an image shared between a build-producer and its pull-only
        siblings must fan out to ALL of them (producer included).

        This is the end-to-end half of the GH-13 regression fix. The
        receiver builds its recipient list from IMAGE_MAP[image_ref];
        if image-map.json on disk omits the producer (an earlier cutover
        ...
        assert set(resp_body["services"]) == {
            ...
            f"services list. Got: {resp_body['services']}. The earlier "
