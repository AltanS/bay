"""Tests for port-drift detection filters (M83-S10).
...
`deploy_accessory.yml` (retired with the per-container loops in M85-S8) and now
serve as the parity oracle for the reconciler's equivalent Python port logic in
`src/bay_reconcile/observe.py`. The pre-M83-S10 inline jinja comparison
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # conflict). Pre-M83-S10 IP-only comparison missed this too.
