...
**Multi-domain support is absorbed into this design.** The `services.yml` schema already accepts any domain string. Traefik already routes by Host header and auto-issues per-domain certs via HTTP-01. No schema or template changes needed — the only gap was split-DNS, which CoreDNS solves (when deployed).
...
### Phase 0: Pre-existing fixes (independent of this plan)
