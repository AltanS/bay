...
- **Incident (2026-04-16)** — `blog` on the `demo` NA region. Build failed,
  ...
  of pushes silently hit the CB guard with no Telegram. Triggered M75-S7
  (CB visibility, CLI surface). Threshold bumped 3→5 in v0.76.1.
...
- **Webhook auto-builds work for `strategy: remote` (v0.76.0+, M75)** —
...
- **Local-strategy producers sharing an image with siblings** —
  ...
  Not in scope for M90 — see issue #13 audit notes in the M90 worklog
  for detail.

### Migration: upgrading past M90

> **Operators upgrading from versions prior to M90:** If your current
...
>
> **Edge case — pre-M90 corrupt mount:** on some hosts a stale
...
> state the M90 render task fails with `Destination ... not writable`
...
> directory). Subsequent restarts via the M90 handler work normally.

### Historical context (GH #13)

The M87 `storefront` consolidation surfaced this gap: the producer
...
config changes never re-rendered the map. Fix landed in M90: the render
...
- **Webhook ownership boundary (M79-S3)** — `bay-webhook` is split
  ...
  `docker_container`. Before M79-S1 this split silently dropped
  compose-snippet changes (the v0.76.0 stale-mount incident). With
  M79-S1's hash-based recreation, compose changes now correctly trigger
