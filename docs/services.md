...
services:
  <name>:
    ...
    log_retention:                   # Opt-in host-side archive (survives container recreation)
      ...
      mode: normal                   #   normal (0640 bay:argo-logreaders) | sensitive (0600 root:root)  # legacy-argo: unix group on hosts, M108 non-goal
...
|---|---|---|---|
| `normal` (default) | `0750 bay:argo-logreaders` | `0640 bay:argo-logreaders` | members of `argo-logreaders` can read |  <!-- legacy-argo: unix group on hosts, M108 non-goal -->
| `sensitive` | `0700 root:root` | `0600 root:root` | `argo-logreaders` is locked out |  <!-- legacy-argo: unix group on hosts, M108 non-goal -->
...
#### `debugbot` is NOT added to `argo-logreaders`  <!-- legacy-argo: unix group on hosts, M108 non-goal -->

The `argo-logreaders` group is created on first opt-in, and `app_user` is added  <!-- legacy-argo: unix group on hosts, M108 non-goal -->
to it automatically. **`debugbot` is not**. Operators must `usermod -aG
argo-logreaders debugbot` manually on hosts where a read-only SSH session  <!-- legacy-argo: unix group on hosts, M108 non-goal -->
...
4. Restrict access to archived logs:
   - Keep the default (`argo-logreaders` group)  <!-- legacy-argo: unix group on hosts, M108 non-goal -->
   - Only add `debugbot` (or other sub-argo-admin accounts) to the group on  <!-- legacy-argo: unix group on hosts, M108 non-goal -->
...
- Not a streaming `--since <duration>` wrapper. `bin/bay logs <svc>
  --since 1h` still forwards to `docker logs` (M50 behavior); date-shaped
...
> **Migration note (M87, framework v0.84.0+):** Pre-M83 the framework auto-rewrote a link target's port from `127.0.0.1:` to `0.0.0.0:`. M83 severed that rewrite to make exposure declarative; M87 closed the resulting gap by adding `ports.expose:` for services and fixing cross-region port resolution. If you have an accessory or service that's a cross-region link target, declare `expose: tailnet` (or `ports.expose: tailnet` for services) explicitly.
...
The fix is `healthcheck_path:` in `services.yml` (M85, framework v0.84.0+):
