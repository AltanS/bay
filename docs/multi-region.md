...
> **Migration note (M87, framework v0.86.0+):** Pre-M83 the framework auto-rewrote a link target's port binding from `127.0.0.1:` to `0.0.0.0:`. M83-S7 severed that rewrite to make exposure declarative; M87 closed the resulting gap by adding `ports.expose:` for services and fixing cross-region port resolution. If you have an accessory or service that's a cross-region link target, declare `expose: tailnet` (or `ports.expose: tailnet` for services) explicitly. Without an explicit `expose:`, the target has no host-port binding on its
