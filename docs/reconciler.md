...
`config_hash` is the same SHA-256 the Ansible config-hash gate stamps:
...
2. Released v0.96.0 (engine), v0.96.1 (infra-path env-digest parity fix), and
   v0.96.2 (access_gateway allowlist append fix). ✓
...
6. **v0.97.0:** retired the per-container Ansible deploy loops
