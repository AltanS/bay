...
def deploy(
    ...
    # ── Post-deploy reachability audit (M83-S11) ─────────────────────
...
# Date-shaped --since (YYYY-MM-DD) implies archive search (M84-S5),
# which requires --path. Duration-shaped values like `1h`, `30m`, `2d`
# still forward to `docker logs --since` as M50 shipped it.
...
def _is_date_shaped(since: str) -> bool:
    ...
    Date-shaped values belong to the archive-search path (M84-S5) and
...
def _archive_path_for(stack_name: str, service: str) -> str:
    """Return the on-host archive directory for a service (M84-S5).
    ...
    (see the M85 split-surface lesson).
    """
...
def _scrub_execute(
    ...
    with the comments here so the M85 split-surface lesson applies (if
...
def logs(
    ...
    # ── --path: archive helper mode (M84-S5) ─────────────────────────
    if path:
        stack_name = _read_stack_name(root)
    ...
    # ── --scrub: GDPR erasure mode (M84-S6) ─────────────────────────
    if scrub:
        if not pattern:
    ...
    # ── default: forward to `docker logs` on the target host (M50) ───
    limit = _resolve_target_host(bay_dir, region)
