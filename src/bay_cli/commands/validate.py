...
# ── Stale config detection constants (M78-S6) ────────────────────────────
...
def _validate_accessory_bindings(
    ...
    """Pre-deploy binding-IP validator (M83-S5).
...
def _validate_link_target_exposure(
    ...
    """Cross-region link target must declare host exposure (M87).
    ...
    For accessory link targets, ``expose: tailnet`` (top-level, M83 shape).
...
def _validate_service_links(
    ...
    Catches the same-stack link trap (M83-S6) that lets a stray `links:`
...
# Reserved for a later phase (M106/D5). Validated-but-rejected so that turning
...
# ── Webhook health probe (opt-in, M91) ───────────────────────────────────
...
def validate(
    ...
    # ── Optional remote config-age check (M78-S6) ──────────────────────
    if check_config_age:
